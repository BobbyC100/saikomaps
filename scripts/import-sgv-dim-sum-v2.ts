/**
 * Script: Import SGV Chinese/Cantonese Dim Sum places (Version 2)
 * Imports places from CSV with Source URLs and creates/updates a map
 */
...
const CSV_PATH = '/Users/bobbyciccaglione/Downloads/sgv-chinese-cantonese-dim-sum_2.csv'
const USER_ID = 'demo-user-id' // Change this if needed
const MAP_SLUG = 'sgv-chinesecantonese-dim-sum-1770424558394' // Use existing map
...
async function main() {
  console.log('🥟 Starting SGV Dim Sum Import (V2 with Source URLs)...\n')
  ...
  if (!list) {
    ...
    console.log(`✅ Created new map: ${list.title}`)
  } else {
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    if (!existingMapPlace) {
      ...
      // Update the user note if it has changed
      if (existingMapPlace.userNote !== (input.comment?.trim() || null)) {
        await db.map_places.update({
          where: { mapId_entityId: { mapId: list.id, entityId: place.id } },
          data: { userNote: input.comment?.trim() || null },
        })
        console.log(`   ✓ Updated note on map`)
      }
    }
