/**
 * Script: Import Comprehensive Westside Infatuation Guide
 * 61 places across Santa Monica, Venice, Culver City, Mar Vista, Palms, West LA, Brentwood
 */
...
const CSV_PATH = '/Users/bobbyciccaglione/Downloads/westside-infatuation-complete.csv'
...
async function main() {
  console.log('🌊 Starting Comprehensive Westside Infatuation Import...\n')
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const neighborhood = fields[2]
    const category = fields[3]
    const comment = fields[4]
    const sourceUrl = fields.slice(5).join(',')
  ...
  const listTitle = 'Westside Complete: Infatuation Guide'
  const slug = `${generateSlug('westside-infatuation')}-${Date.now()}`
