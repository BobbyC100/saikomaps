...
Confirms `OperatorPlaceCandidate` audit columns (`reviewed_at`, `approved_by`, `confidence_bucket`, `match_score`) and `entity_job_log` table exist. Includes quick counts by `confidence_bucket`.
