/**
 * Script: Import SGV Curated Places (Master List)
 * Imports comprehensive curated list from CSV with categories and creates a master map
 */
...
async function main() {
  console.log('🗺️  Starting SGV Curated Master List Import...\n')
  ...
  // Parse each data line manually (handles quoted fields with commas)
  for (let i = 1; i < lines.length; i++) {
    ...
    if (fields.length < 4) continue // Need at least Name, Address, Category, Comment
    
    const name = fields[0]
    const address = fields[1]
    const category = fields[2]
    const comment = fields[3]
    const sourceUrl = fields.slice(4).join(',')
  ...
  const listTitle = 'SGV Master Guide'
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    if (!place) {
      ...
      if (placeDetails) {
        enriched++
        console.log(`   ✓ Created: ${place.name} [${input.category}]`)
      } else {
        failed++
      ...
      existing++
      // Update with new source if available
      ...
      if (newSource && !existingSources.some((s: any) => s.url === newSource.url)) {
        ...
        console.log(`   ↻ Updated: ${place.name} [${input.category}]`)
      } else {
        console.log(`   ↻ Exists: ${place.name} [${input.category}]`)
      }
    }
...
function extractSourceName(url: string): string {
  ...
  if (url.includes('sgvtribune.com')) return 'SGV Tribune'
  if (url.includes('visitpasadena.com')) return 'Visit Pasadena'
  if (url.includes('fodors.com')) return "Fodor's"
